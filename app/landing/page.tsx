  ...
  return (
    <div className="min-h-screen bg-clay-100 text-ink">
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 py-8">
        <nav className="flex items-center justify-between border-b border-hairline pb-5">
          <Link href="/" className="font-mono text-[15px] font-bold tracking-wider">
          ...
          <div className="flex items-center gap-3 text-sm font-medium">
            ...
            <Link
              href="/signup"
              className="rounded-md bg-celadon-600 px-4 py-2 text-clay-50 hover:bg-celadon-700"
            >
              시작하기
            </Link>
          </div>
        ...
        <section className="grid items-center gap-10 py-14 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="flex max-w-2xl flex-col gap-7">
            ...
            <h1 className="break-keep text-4xl font-semibold leading-tight tracking-tight text-ink sm:text-6xl">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link
                href="/signup"
                className="flex h-12 items-center justify-center rounded-md bg-celadon-600 px-5 text-sm font-semibold text-clay-50 hover:bg-celadon-700"
              >
                무료로 시작
              </Link>
              <Link
                href="/"
                className="flex h-12 items-center justify-center rounded-md border border-hairline-strong px-5 text-sm font-semibold text-ink hover:bg-clay-50"
              >
                서비스 홈으로
              </Link>
            </div>
        ...
        <section className="flex flex-col gap-6 border-t border-hairline py-14">
          <div className="flex flex-col gap-2">
            <p className="font-mono text-xs font-medium uppercase tracking-[0.2em] text-celadon-600">
              How it works
            </p>
            <h2 className="break-keep text-2xl font-semibold tracking-tight text-ink sm:text-3xl">
              3단계면 충분합니다
            </h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            {HOW_IT_WORKS.map((item) => (
              <div key={item.step} className="rounded-lg border border-hairline bg-clay-50 p-5">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-ink font-mono text-sm font-semibold text-clay-50">
                  {item.step}
                </span>
                <p className="mt-4 text-base font-semibold text-ink">{item.title}</p>
                <p className="mt-1 text-sm leading-6 text-ink-dim">{item.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="flex flex-col gap-6 border-t border-hairline py-14">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {QUALITY_TIERS.map((tier) => (
              <div key={tier.grade} className="rounded-lg border border-hairline bg-clay-50 p-5">
                <div className="flex items-center gap-2">
                  <span className="flex h-9 w-9 items-center justify-center rounded-md bg-ink font-mono text-sm font-bold text-clay-50">
                    {tier.grade}
                  </span>
                  <p className="text-sm font-semibold text-ink">{tier.input}</p>
                </div>
                <p className="mt-3 text-sm leading-6 text-ink-dim">{tier.detail}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="flex flex-col gap-6 border-t border-hairline py-14">
          ...
          <div className="divide-y divide-hairline rounded-lg border border-hairline bg-clay-50">
            {FAQ_ITEMS.map((item) => (
              <details key={item.question} className="group p-5">
                ...
                <p className="mt-3 text-sm leading-6 text-ink-dim">{item.answer}</p>
              </details>
            ))}
          </div>
        </section>

        <section className="flex flex-col items-center gap-4 rounded-2xl border border-hairline bg-clay-50 px-6 py-14 text-center">
          <h2 className="break-keep text-2xl font-semibold tracking-tight text-ink sm:text-3xl">
        ...
        <footer className="flex flex-col gap-2 border-t border-hairline py-8 text-sm text-ink-dim sm:flex-row sm:items-center sm:justify-between">
